...
for j = 1:length(plt_types)
  for batch_size = [200, 400]
    for i = 1:9
      ...
      ptsne_data(:,i) = fscanf(fopen("old_output/" + plt_types(j) + "_ptsne" + s, 'r'), '%f', 150);
      vptsne_data(:,i) = fscanf(fopen("old_output/" + plt_types(j) + "_vptsne" + s, 'r'), '%f', 150);
    end
    subplot(120 + j)
    hold on
    plterr(vptsne_data.', 1:10:1500, "VPTSNE " + batch_size)
    plterr(ptsne_data.', 1:10:1500, "PTSNE " + batch_size)
    title(plt_titles(j))
